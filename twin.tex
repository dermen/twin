...
In a typical exposure, most NPs will not scatter into the detector, and the ones that do will not be oriented such that they scatter into multiple detectable directions. Therefore, the average exposure is comprised of a large fraction of randomly scattered photons (owing to the orientation randomness in  a solution). As such, the signal-to-noise of a single exposure is much less than unity and scales as the square root of the number of averaged exposures, $N_s$ [12]. We consider an exposure as a snapshot, meaning the NPs should not be moving significantly throughout the exposure duration. This is accomplished automatically with the femtosecond timescale pulses of the SACLA facility [13]. However, SCXS can be conducted at synchrotron facilities, provided  that the sample is prepared accordingly, and that it is cooled during exposure [14,15].

\section{Experimental}
60 nanometer Gold NPs were purchased from (NP company) and suspended in LCP at a concentration of 40 mg/mL. The viscous solution was injected into the beam as a constant stream using a (diameter) syringe. The so-called toothpaste injector oozes solution at a relatively slow rate of (rate).

The SACLA beam was focused down to a roughly $25 − 50 \mu m ^2$ spot size, and the beam repetition rate was 30 Hz. the femto-second pulses were measured using a MPCCD 8-panel detector. Gain calibrations were provided by SACLA, and dark current measurements were performed every 5000 exposures and used to adjust for electronic noise natural to CCD detectors. With this setup we acquired 200,000 exposures of gold NPs.
