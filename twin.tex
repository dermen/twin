...
Note, the maximum angle $\psi$ that one can detect $\{111\}$ correlations is measured in pixels whose azimuthal separation $\Delta=180^o$, hence

\be \label{psimax}
\cos(\psi_{max}) = - \cos(2\theta_{111})
\ee

With this in mind, it is easily shown that for defect-free gold NPs (modeled as bulk FCC domains with spherical boundaries), one should only expect correlations to arise at values $\cos(\psi) = \pm (1/3)$. For example, 
...
(Figure \ref{fig:contrast}). Note that these are supplementary angles, which is a direct consequence of Friedel symmetry [11] between $\bm q_{11\bar{1}}$ and $\bm q_{\bar{1}\bar{1}1}$. All other combinations of $\bm q_1, \bm q_2 \ne \bm q_1 \in \bm Q_{111}$ will give $\cos(\psi) = -1$, corresponding to an un-measurable correlation. However, it is known that smaller FCC NPs tend to form complicated twinning structures [ref], composed of many tetrahedral sub-units. The reciprocal space of these objects is more complex than that of bulk FCC NPs, however this complexity is hidden in standard powder diffraction images because the twins all scatter into the same Bragg ring, making it impossible to distinguish a twin from a separate NP. CXS, however, is sensitive to twinned NPs. Consider the following simple model for tetrahedral nearest-neighbor FCC twins (Fig 1c). The twins (twinA and twinB) have the same internal lattice structure, however one is rotated relative to the other in a way that preserves a common crystallographic direction. As a typical example, consider twinA to be related to twinB via a rotation of $180^{o}$ about the $(111)$ direction of twinA, given by the matrix
...
We can use this matrix to show how correlations can arise at angles corresponding to inter-twin double Bragg correlations:

\be
\frac{ \bm q^A_{\bar{1}\bar{1}1} \cdot \mathbf T \cdot \bm q^B_{1\bar{1}1}} {{q^A_{\bar 1 \bar 1 1} q^B_{1\bar{1}1} }} \,=\, \frac { ( −1,-1,1 ) \cdot \mathbf T \cdot ( 1,-1,1 ) } {3} \,=\, \frac{5}{9}
\ee

and similary

\be
\frac{ \bm q^A_{11 \bar{1}} \cdot \bm T \cdot \bm q^B_{\bar{1}\bar{1}1}} { { q_{11\bar 1}  q_{ \bar 1 \bar 1 1} } } \,=\, \frac{7}{9}
\ee

For this simple model of two twinned FCC domains, it is a simple matter to show that correlations will arise at $\cos (\psi) = \pm(1/3),\, \pm (5/9),\,  \pm (7/9)$ (Figure \ref{fig:contrast}). The largest angle here is $\arccos( -7/9)$, hence according to (\ref{psimax}), in order to measure a full spectrum of correlations from nearest neighbor twins, one must have $2\theta \ge -7/9$

Depending on the formation process, gold NPs have been observed in several twinned shapes, including decahedral and icosahedral. In a decahedron, for instance, there are 10 tetrahedral units which fit together at the twin boundaries. The tetrahedral units themselves are FCC crystals, whose boundaries are $\{111\}$ planes [ref].
